...
**The Weyl split** *(used in Ch. 18, 25)*: Riemann decomposes into the Ricci part (fixed pointwise by matter through (16.2)) and the trace-free **Weyl tensor** $C^{\mu}{}_{\nu\rho\sigma}$ — the *free* part of geometry, which propagates: tidal fields across empty space, light bending, gravitational waves all live in Weyl. Conformally flat 4-metrics ($g = \Omega^2\eta$) have $C \equiv 0$. (One dimensional fine print, load-bearing for Ch. 18: in *three* dimensions the Weyl tensor vanishes identically for every metric, and conformal flatness is diagnosed by the **Cotton tensor** instead — a distinction Ch. 18's obstruction theorem must, and does, respect.)
...
**Parker's particle production.** A field mode on FRW obeys, in conformal time, an oscillator equation with time-dependent frequency $\omega_\eta^2 = k^2 + m^2 a^2(\eta)$ — the conformally coupled form (minimal coupling adds a $-a''/a$ term; the distinction is immaterial for this thesis's fermionic matter, conformally invariant when massless) — comoving $k$ constant, all time dependence through the scale factor; measured in cosmic time the physical frequency is $\omega = k/a$, the redshift of §16.5. Two regimes, set by the now-familiar dial: modes with $\omega \gg |\dot a/a|$ track adiabatically (no quanta produced — the WKB vacuum rides along); modes with $\omega \lesssim |\dot a/a|$ cannot track — their "vacuum" before and after differ, related by a Bogoliubov transformation, and the *out* observer counts quanta in the *in* vacuum: **the expansion of space creates particles**, mode by mode, exactly where adiabaticity fails. This is Ch. 5's boxed principle wearing its original (1968) clothes, and the sudden quenches of Part II are its step-function limit — a point Ch. 17 makes into a dictionary entry rather than an analogy.

**Moving mirrors (Davies–Fulling).** In 1+1D every metric is conformally flat ($ds^2 = \Omega^2(dt^2 - dx^2)$ — two metric functions, two coordinate freedoms), and massless fields are conformally invariant — so a field bounded by *moving mirrors* in flat space reproduces the physics a massless field would have in a curved 2D spacetime: the conformal factor is invisible to it, all the physics sits in boundary and asymptotic structure, and the mirror trajectory encodes exactly that — trajectory ↔ geometry, up to conformal equivalence. Accelerating mirrors radiate (the flat-space shadow of Hawking radiation); the only conformal-symmetry breaker is a mass, entering through the invariant combination $m\Omega$ — the reason Part II's physics organized itself into the single variable $mL$, as Ch. 17 will note with satisfaction.
