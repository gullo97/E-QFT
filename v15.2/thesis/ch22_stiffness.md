...
*Notation: from §22.2 on, $s$ denotes the proper-time regulator (App. D, ruling D.5), not an expansion ratio.*
...
**Cell energetics.** Add an occupant in mode $n$ — with a declared modeling choice **[Postulate]** that deserves its own sentence, because the stability result depends on it: the occupant is a *massive, nonrelativistic* particle carrying Part I's spectrum $E_m = \pi^2 n^2/2mL^2$ (henceforth $m = 1$ units), while the vacuum term is the *massless* species' Casimir energy — two different fields, the minimal cell content with both an attractive vacuum and a stiff occupant. Neither alone suffices: a single massless field would make the total scale-free ($E \propto 1/L$, monotone — no equilibrium, no occupied/empty dichotomy), and a single heavy field has exponentially suppressed Casimir energy. The hybrid is self-consistent at its own equilibrium: the occupant's momentum there is $n\pi/L^\ast = 1/24n \ll m = 1$, comfortably nonrelativistic. Total and equilibrium:
...
$$\frac{\Pi(q)}{\hat q^2} \;\in\; [-0.18,\, -0.05] \;\text{(eleven negative modes)}, \qquad \text{median } \kappa^{\text{vac}}_{\text{grad}} \;\approx\; -0.13, \tag{22.3}$$

negative for eleven of twelve modes — the single positive outlier ($+0.06$ at $k = 2$) and the odd/even scatter are a fixed-endpoint parity effect, reported as is; the quoted median is over the long-wavelength half $k \le 8$ ($-0.14$ over all twelve). Meanwhile one occupant of mode $n$ per cell contributes, by a single Taylor expansion of $\sum_i \pi^2n^2/2L_i^2$ in $L_i = 1 + u_i - u_{i-1}$,

$$\kappa^{\text{matter}}_{\text{grad}} \;=\; +\,3\pi^2 n^2 \;=\; +29.6 \;\;(n = 1) \tag{22.4}$$

per bond — positive and two orders of magnitude larger.

**What the sign means.** First, why this number is trustworthy at all: unlike the 3+1D Einstein–Hilbert coefficient (quadratically divergent, hence regulator-scheme-dependent — sign included; the audit is Ch. 25.5), the 1D response is a *finite* Casimir-type difference, Richardson-extrapolated to $s \to 0$: it has no scheme ambiguity. It is the framework's regulator-independent anchor for the sign. And the sign says: the matter vacuum makes the *compression* mode of geometry unstable — bending the cell sizes into a wave lowers the vacuum energy. That one measured sign is simultaneously three known facts about gravity, here *derived*: (i) **gravity is attractive** — compressions grow; the Jeans instability of §22.1 seen now in the effective action; (ii) **the conformal mode of the gravitational action has wrong-sign kinetic term** — the notorious conformal-factor problem of Euclidean gravity, an *output* of matter loops in this model, and the dynamical origin of the Lorentzian signature that Ch. 19 honestly deferred; (iii) **ordinary matter stabilizes geometry at short scales** — occupied cells are stiff (22.4), which is why space does not crumple around matter. The constraint's negative geometry-energy (Ch. 21) is hereby a measurement.
...
**The cutoff, identified — and derived.** In generic induced-gravity programs, $\Lambda_{\text{UV}}$ is the embarrassment: an arbitrary scale on which $G$ depends quadratically. The box model has no such freedom, and the reason can be exhibited *exactly*, not argued. Take the model's actual microstructure in its simplest habitat: a one-dimensional lattice of cells of size $\bar L$, whose Laplacian has the band dispersion $\omega^2(k) = (4/\bar L^2)\sin^2(k\bar L/2)$, $k$ restricted to the Brillouin zone. Its heat-kernel trace per cell is a closed form,

$$\operatorname{tr} e^{-s\,\omega^2}\Big|_{\text{per cell}} \;=\; \int_{-\pi/\bar L}^{\pi/\bar L}\frac{\bar L\,dk}{2\pi}\,e^{-s\omega^2(k)} \;=\; e^{-z}\,I_0(z), \qquad z \;=\; \frac{2s}{\bar L^2}, \tag{22.5b}$$

with $I_0$ the modified Bessel function. Its two regimes are the entire story of the cutoff:

- **$s \gg \bar L^2$ (coarse probe):** $e^{-z}I_0(z) \to \frac{\bar L}{\sqrt{4\pi s}}\big[1 + \frac{\bar L^2}{16 s} + \cdots\big]$ — the continuum Weyl kernel, plus corrections that measure the probe's ability to resolve cells. The continuum expansion (22.5) is exactly this regime.
- **$s \ll \bar L^2$ (fine probe):** $e^{-z}I_0(z) \to 1$ — the trace **saturates at one mode per cell**. The continuum kernel, extrapolated here, counts modes the lattice does not have; the $s \to 0$ divergence of (22.5) is a fiction of the continuum description.

The proper-time integral in (22.5) must therefore be cut at $s_{\min} \sim \bar L^2$ — not by choice, but because that is where its integrand stops describing the system. Writing $s_{\min} = (\bar L/c)^2$ defines $\Lambda_{\text{UV}} = c/\bar L$, with $c$ an $\mathcal O(1)$ constant that is a *computable property of the lattice*, not philosophy: a sharp mode-counting match (continuum modes below $\Lambda$ = lattice modes: $\Lambda^3/6\pi^2 = \bar L^{-3}$) gives $c = (6\pi^2)^{1/3} \approx 3.9$ for a cubic lattice; a heat-kernel-weighted match gives a smaller $\mathcal O(1)$ value. Pinning $c$ for the model's actual cell ensemble, with the spin-weighted species sum, is the finite calculation of Ch. 27, item 2 — and the spread in $c$ under reasonable matching criteria (roughly $1$ to $4$) is the honest uncertainty bar on everything downstream of (22.7). Two consequences:

> **Planck-Cell Prediction [Theorem, given the induced-gravity postulate + the cutoff identification].** Inverting $G_{\text{ind}}^{-1} = N_{\text{eff}}c^2/12\pi\bar L^2$:
>
> $$\bar L \;=\; c\,\sqrt{\frac{N_{\text{eff}}}{12\pi}}\;\ell_P \;\approx\; 1.6\,c\;\ell_P \quad (N_{\text{eff}} \sim 100). \tag{22.7}$$
>
> The geometry's granularity sits at the Planck scale — self-consistently: a smaller cell would strengthen gravity, which would shrink the Planck length, in just the right proportion.

**What kind of statement (22.7) is — the audit.** Precision here matters more than rhetoric, because (22.7) is the closest thing this thesis has to a "size of the universe's voxel", and it is easy to over-read. Three premises go in:

1. **The induced-gravity postulate** (Sakharov's): the geometry has *no bare stiffness of its own* — every unit of $G^{-1}$ comes from matter loops. In this framework the premise is not gratuitous: Part I gave the cells no elastic action, only matter and transition rules, and §22.1–22.2 *measured* the stiffness appearing from the matter vacuum on cue. But as a statement about 3+1D it remains a structural **[Postulate]**: nothing forbids a bare term; the framework simply does not write one.
2. **The cutoff identification** $\Lambda_{\text{UV}} = c/\bar L$, now derived from the saturation of the cell-lattice heat kernel (22.5b) — the strongest link in the chain.
3. **The species count** $N_{\text{eff}} \sim 100$, imported from the Standard Model **[Standard]**, with its spin weights unresolved (Ch. 27, item 2).

Given 1–3, equation (22.7) is a *consistency inversion*, not an independent measurement: $\ell_P$ is defined from the observed $G$, so once all of $G$ is induced at the cell scale, dimensional analysis forces $\bar L \sim \sqrt{N_{\text{eff}}}\,\ell_P$ — the outcome could never have landed at the electroweak or Hubble scale unless $N_{\text{eff}}$ were absurd ($10^{34}$ species). The honest content of (22.7) is therefore *not* the numerical coincidence but three sharper statements. First, **the sign and existence of induced stiffness are genuine dynamical outputs** — §22.2's measured $\kappa^{\text{vac}}_{\text{grad}} < 0$ could have come out positive (repulsive gravity) or zero (no induced dynamics), and either would have killed the program; the scale then follows from the two premises. Second, **the framework closes a loop that generic induced gravity leaves open**: Sakharov's construction needs a physical cutoff and has none to offer; a discrete-geometry model needs its discreteness scale fixed and has no way to choose it. Each supplies exactly the other's missing piece, with no third scale left dangling. Third, the relation is **falsifiable at its joints rather than at its output**: premise 1 fails if a bare stiffness is required elsewhere (e.g., if the induced value's species-dependence contradicts the universality of $G$ — a version of the classic objection to induced gravity, inherited here and owed an answer at the same address as $N_{\text{eff}}$); premise 2 fails if the measured covariance window of Ch. 25 had not matched the cell-scale cutoff — it did.

**The voxel, read correctly.** "Cells of Planck size" invites the picture of a rigid crystal pixelating space — the picture is wrong on three counts, each already established. The cell sizes are *dynamical quantum variables* (Part I): $\bar L$ is the mean of a distribution, with fluctuations that are themselves the conformal-mode physics of §22.2. The lattice has no preferred rest frame built in — but whether Lorentz invariance is exact or merely accidental at accessible energies is a genuine open question with observable stakes (Planck-scale dispersion of light and gravitational waves; Ch. 27, item 10 keeps it honest and modest). And the cells are not *objects* but sectors of a Hilbert-space decomposition (Ch. 3): "voxel" describes the bookkeeping of geometry states, not furniture sitting in a pre-existing space.
...
The stiffness is no longer a unit choice. Bottom-up: the 1D induced action is *measured* — attractive Casimir potential (22.1), vacuum Jeans instability with the chain collapsing on cue, gradient coupling $\kappa^{\text{vac}}_{\text{grad}} \approx -0.13 < 0$ against matter's $+3\pi^2n^2$ — delivering the attraction, the conformal-factor sign, and matter stabilization as outputs. Top-down: Sakharov's integral with the cell-scale cutoff gives $G_{\text{ind}}^{-1} = N_{\text{eff}}\Lambda^2/12\pi$ and the **Planck-Cell Prediction** $\bar L \approx 1.6\,c\,\ell_P$ — computed in the 4D-covariant proper-time scheme, which Ch. 25.5's sign audit identifies as the scheme in which matching to the covariant action is legitimate (and in which the 3D lattice measurement returns gravity's absolute signs, $G_{\text{ind}} > 0$). The constraint of Ch. 21 now stands on measured ground, and two chapters of spending await: the static limit (Newton's law, Ch. 24) and the propagating limit (the graviton with its universal coefficient, Ch. 25) — with the gauge-theoretic consolidation (Ch. 23) in between.

---

**Validation.** `ch22_casimir_chain.py`: cell energetics, equilibria, the 12-cell relaxation (endpoint values $0.135$/$3.865$/$24.000$, asserted programmatically; core-scale ratio $0.956$), regenerates Fig. 22.2. `ch22_dispersion.py`: the semi-transparent-wall vacuum dispersion (eleven-mode band $[-0.18, -0.05]$ with the $+0.06$ parity outlier, medians $-0.13$ over $k \le 8$ / $-0.14$ over all twelve; matter line $+3\pi^2 n^2$), writes `data/ch22_stiffness_dispersion.json`, regenerates Fig. 22.3. (Both adapted from the archived `scripts/ports/` originals.) App. B re-derives (22.5)–(22.6) symbolically. All quoted numbers printed by the scripts.
