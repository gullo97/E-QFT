...
![Shape space](figures/ch19_fig1_shape_space.png)

*Figure 19.1 — Shape space. The six metric directions per cell: the diagonal sector (volume and stretches — flat in the fidelity metric but energy-carrying) and the shear sector (energy-flat but geometry-carrying) — the segregation the Splitting Theorem proves. (drawn)*
...
**[Computed]** (`ch19_fidelity.py`, base configuration $A = \mathrm{diag}(1, 1.21)$): diagonal-sector fidelity at machine zero — $|\chi| \lesssim 10^{-10}$ (conformal and traceless stretch, Richardson-combined) across all modes tested, $-5\times10^{-13}$ in the dedicated conformal check; shear-sector fidelity finite and mode-structured — $\chi = 0.049,\ 5.32,\ 5.32,\ 3.73$ for the computed modes $n = 0, 1, 2, 4$ — agreeing with the perturbation-theory form (19.2) to four significant digits; shear energy slopes consistent with zero at first order (residuals $\sim \varepsilon\,\times$ curvature).
...
**Validation.** `ch19_fidelity.py` (adapted from the archived `scripts/ports/dewitt_fidelity.py`): direct-overlap vs perturbative fidelity (four digits); the machine-zero diagonal sector ($\chi = -5\times10^{-13}$) and finite shear sector values quoted above; shear energy-slope nulls; writes `data/ch19_dewitt_fidelity.json` and regenerates Fig. 19.2. Figure 19.1 (shape-space schematic: `ch19_fig1_shape_space.png`, drawn, `make_schematics.py`).
