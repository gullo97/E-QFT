...
The asymmetry is conventionally quoted as the baryon-to-photon ratio, measured two independent ways. The cosmic microwave background's acoustic peaks fix the baryon density (the second-to-first peak ratio is the sensitive dial), giving $\eta_B = (6.12 \pm 0.04)\times10^{-10}$ (Planck 2018, from $\Omega_b h^2 = 0.02237 \pm 0.00015$). Big-bang nucleosynthesis reads the same number from primordial abundances — deuterium above all, whose survival is exponentially sensitive to the baryon density at $T \sim 0.1$ MeV — and concords: the BBN+CMB concordance value is $(6.14 \pm 0.19)\times10^{-10}$, the number this thesis adopts as its target. Two epochs separated by 400{,}000 years of cosmic history, one number: $\eta_B$ is among the most secure quantities in cosmology.

Why is *nonzero but tiny* the puzzle, in both directions? Suppose the universe were exactly symmetric. Nucleons and antinucleons stay in equilibrium until annihilation $N\bar N \to$ pions outruns the expansion; freeze-out happens at $T \approx 20$ MeV, when the equilibrium abundance carries the Boltzmann penalty $e^{-m_N/T} \approx e^{-47}$. The relic that survives annihilation is set by the freeze-out balance $n\,\langle\sigma v\rangle \sim H$ — the ratio of the Hubble rate to the annihilation cross-section — and the standard estimate lands at

$$\frac{n_N}{n_\gamma}\bigg|_{\text{symmetric}} \sim 10^{-18}\text{–}10^{-19}$$

— *eight to nine orders of magnitude* too little matter, and equal antimatter alongside it (which the absence of annihilation gamma-ray signatures from galaxy clusters independently excludes at any interesting scale). A symmetric universe is not approximately our universe; it is empty. Conversely $\eta_B \sim 1$ would have rewritten nucleosynthesis and structure formation entirely. The number $6\times10^{-10}$ is a *target*, not an impression: one excess baryon per ~$1.6$ billion photons in today's universe.
...
$$\partial_\mu j^\mu_{B+L} = \frac{3\,g^2}{16\pi^2}\,W^a_{\mu\nu}\tilde W^{a\,\mu\nu} \quad (\text{while } B - L \text{ stays exact}),$$
...
Two distinct conversion factors now enter, and they must not be conflated. **First**, a mechanism that delivers a number density at the production epoch — naturally quoted per $T^3$ — converts to the frozen ratio via the entropy formula at that epoch:

$$\frac{n_B}{s} \;=\; \frac{45}{2\pi^2 g_*}\;\frac{n_B}{T^3}\bigg|_{\text{prod}}, \qquad \frac{45}{2\pi^2 g_*} = 0.02136 \quad (g_* = 106.75).$$

**Second**, the frozen $n_B/s$ converts to today's baryons-per-photon through the *present* entropy-to-photon ratio (this is where the $e^+e^-$ heating of photons after neutrino decoupling lives, via $g_{*s}^{\text{today}} = 2 + \tfrac78\cdot 6\cdot\tfrac{4}{11} = \tfrac{43}{11} = 3.91$):

$$\eta_B \;=\; \frac{n_B}{s}\,\times\,\frac{s}{n_\gamma}\bigg|_{\text{today}}, \qquad \frac{s}{n_\gamma}\bigg|_{\text{today}} = \frac{\pi^4}{45\,\zeta(3)}\,g_{*s}^{\text{today}} \;=\; 7.04.$$

The full chain from a production-epoch density to today's observable is therefore

$$\eta_B \;=\; 7.04 \times \frac{45}{2\pi^2 g_*} \times \frac{n_B}{T^3}\bigg|_{\text{prod}} \;=\; 0.150\;\frac{n_B}{T^3}\bigg|_{\text{prod}}$$

(before the sphaleron conversion factor of §13.3, which acts on the charge stored upstream of it). Equivalently: the photon-normalized asymmetry is diluted between production and today by $\big(s/n_\gamma\big)_{\text{prod}}\big/\big(s/n_\gamma\big)_{\text{today}} = 1.80\,g_*/7.04 = 27.3$ — about one-and-a-half orders of magnitude, *everything else that was hot* — and this factor is not negotiable by model-building. One further assumption is thesis-specific rather than standard, and is flagged as such: converting a *per-cell* production probability into a density $n_B/T^3$ at the epoch presumes that the relevant cell density scales as $T^3$ there — **[Postulate]**, made explicit where it is used (Ch. 14).
...
| Bookkeeping to today | entropy chain $7.04 \times 45/(2\pi^2 g_*) = 0.150$ per unit $n_B/T^3$ at production | step/window counting $\to \mathcal P_\times$ (Ch. 14) | **[Standard]** + **[Theorem]** |

With the standard pieces audited and the division of labor explicit, the assembly can proceed.

---

**Validation.** `ch14_eta_assembly.py` prints the audited constants used here ($28/79 = 0.3544$; $45/2\pi^2g_* = 0.02136$ at $g_* = 106.75$). The present-day factor $s/n_\gamma|_{\text{today}} = 7.04$ is exact arithmetic from $g_{*s}^{\text{today}} = 43/11$ via the displayed formulas; whether the Ch. 14 assembly carries it explicitly or absorbs it into its efficiency definitions belongs to that chapter's audit. Figures: `ch13_fig1_sakharov_map.png`, `ch13_fig2_ewbg_failure_modes.png` (drawn schematics).
