...
Invariance, term by term — the only subtle entry being the famous conspiracy of the first two: $(D\sigma)^2$ has weight zero by counting ($\sqrt{-g}$: $+4$; two $D\sigma$: $-2$; inverse metric: $-2$), *but* the $W$-free part $(\partial\sigma)^2$ produces inhomogeneous $\partial\Omega$ terms; meanwhile $R\sigma^2$ produces inhomogeneous $\Box\Omega$ terms by (23.4). With the relative coefficient $-\tfrac12 : -\tfrac{1}{12}$ — and only with it — the two inhomogeneities cancel after one integration by parts: this is the "improved" (conformal) coupling of Callan–Coleman–Jackiw, rederived rather than cited. The $\sigma^4$ term: weight $4 - 4 = 0$, the unique invariant polynomial. The $F^2$ term: gauge-invariant by construction, weight zero by the same counting as Maxwell. **[Standard]** (Deser 1970; Dirac 1973; modern treatments: 't Hooft, Ghilencea.)
...
2. **A massive Weyl vector**: the $(D\sigma)^2$ term at constant $\sigma$ is $-\tfrac12\sigma_0^2 W^2$ as displayed — a Higgs-style mass $M_W^2 = e_W^2\sigma_0^2 = 6e_W^2M_P^2$ for the would-be photon-of-scale. One sign here is owed an explicit audit rather than silence **[Open, one line]**: against the healthy Proca form in this signature ($-\tfrac14F^2 + \tfrac12M^2W_\mu W^\mu$), the displayed mass term carries the opposite sign, inherited directly from the compensator's wrong-sign kinetic term — the standard treatments (Dirac 1973; Ghilencea) obtain a healthy massive vector, and reconciling the convention chain (or showing the sign is tamed by the same constraint that tames the conformal sector, of which $W$ is the gauge field) is a bookkeeping check of exactly the Ch. 25.5 kind. Either way the vector decouples in the infrared at the scale $|M_W|$, leaving pure Einstein gravity below it. (Einstein's 1918 objection is thereby answered rather than dodged: the second clock effect is suppressed by the $W$ mass.)
