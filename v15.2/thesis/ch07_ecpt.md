...
>
> **(ii) Dynamically.** With $N$ spectators, the Expansion-Preference Theorem — under Ch. 6's stated conditions: sector arena, embedding coupling, short-time regime — gives $P_{\text{con}}/P_{\text{exp}} = (1 - n_1^{-2})\prod_\alpha \mathcal N_{\text{con}} \to 0$ exponentially in $N$: even far from the floor, the measure of quantum mechanics is one-sided.
...
*Figure 7.1 — The corner of the $\mathbb Z_2^4$ subgroup lattice relevant to the conjecture. Solid: $CPT$, exact **within each geometry sector** (fixed geometry — the setting of the CPT theorem), and conjecturally $ECPT$, exact on the full inter-sector dynamics. Broken: $E$ (Ch. 6), $P$, $C$, $CP$, $T$ (Standard Model) — and, on the full inter-sector dynamics, $CPT$ itself, forced by group multiplication: $E = (ECPT)\cdot(CPT)$, so if $ECPT$ is exact and $E$ is broken, full-dynamics $CPT$ must break with it (§7.5; the within-sector qualifier is what keeps the fixed-geometry CPT theorem untouched).*
...
$$\underbrace{ECPT \text{ exact}}_{\text{[Conjecture]}} \;\wedge\; \underbrace{E \text{ broken}}_{\text{[Theorem], Ch. 6}} \;\wedge\; \underbrace{T \text{ unbroken in the traffic}}_{\text{[Postulate], this section}} \;\;\Longrightarrow\;\; CP \text{ broken, correlated with } E. \tag{7.3}$$

The logic, stated with its joints visible. As group elements $ECPT = E \cdot (CPT)$, so exactness of any two of $\{E,\, CPT,\, ECPT\}$ forces the third; contrapositively, if $ECPT$ is exact (the conjecture) and $E$ is broken (the Ch. 6 theorem), then **$CPT$ must be broken on the full dynamics** — the fixed-geometry CPT theorem **[Standard]** is not contradicted, because it governs the physics within a sector, not the inter-sector traffic. Where inside $CPT$ the compensating breaking falls is *not* fixed by group theory; it takes one further input. If the geometry traffic realizes time reversal without additional breaking — an assumption we state rather than smuggle, hence the third slot in (7.3) — the compensation falls on $CP$: the $E$-breaking bias of geometry traffic must be accompanied by a $CP$-breaking bias of the matter transported by that traffic, with the two biases locked in sign. The chain (7.3) is only as strong as its weakest link, and each link now carries its own label.
...
**Validation.** No numerics native to this chapter. The oddness property $\Delta Q(+\delta) + \Delta Q(-\delta) = 0$ is reported with Ch. 11's computations; the wall-CP machinery it lives in is exercised by the existing Ch. 11–12 suite (`ch11_wall_density.py`, `ch12_pump_control.py`) — and, as §7.4 says plainly, it is a consistency check rather than discriminating evidence. Figures 7.1–7.2 are drawn schematics generated by `make_schematics.py`.
