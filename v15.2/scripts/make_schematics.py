...
def ch18_fig2():
    ...
    box(ax, 0.3, 1.0, 3.2, 1.0, "frame rotations (3)\n$SO(3)$: pure gauge\n(rigid ambient rotations)", fc="#f4f4f4", ec=GRAY, fs=8.5)
    box(ax, 4.2, 1.0, 3.2, 1.0, "closure: $\\mathfrak{gl}(3)$\nsymmetric 6 = metric\nantisymmetric 3 = gauge", fc="#f3eefb", ec=PURPLE, fs=8.5)
    ax.text(3.85, 0.45, "the $\\times$ polarization / TT completeness lives in the shear sector:\n"
                        "dilations alone $\\Rightarrow$ diagonal-only geometry, no rotational covariance (Obstruction Theorem)",
...
def ch18_fig3():
    ...
    ax.text(1.4, 0.45, "dilations only:\naxis-locked shapes (diagonal $g_{ab}$)\nCotton-flat isotropic subsector", fontsize=8.5, ha="center")
    ...
    ax.text(5.3, 0.45, "with shears:\nshapes tilt — edge angles move,\nfull TT sector available", fontsize=8.5, ha="center", color=GREEN)
    ax.text(3.35, 3.3, "LIGO's $\\times$ polarization lives here $\\rightarrow$", fontsize=8.5, color=ORANGE)
...
# ch25_fig2 (covariance window) is a DATA figure: generated by ch25_graviton.py
# from the live window scan — no longer hand-drawn here.
