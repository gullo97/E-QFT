...
`python run_all.py --report` additionally captures each script's printed output
and writes the provenance table (script -> tier -> what it validates -> status
-> captured numbers) to ../data/provenance.json (App. C.6, App. D.7).
"""
import argparse
import json
import pathlib
...
REGISTRY = [
    ...
    ("ch03_overlaps.py",          "fast",  "overlap closed forms; dilation matrix elements 10.4762/40.4938/160.4984"),
    ...
    ("ch06_norms.py",             "fast",  "norm theorems; N_con table; fermion Gram det 0.2320<=0.3895; asymmetry vs N"),
    ...
    ("ch10_slab_check.py",        "fast",  "slab eigenvalue theorem at randomized (k_y,k_z); 4-component solver"),
    ...
    ("ch12_chiral_bag.py",        "slow",  "rays 2e-16; master eq 7e-8; Q_vac law (-0.4455 @ D=2.8); plateaus/jump; truncated forms"),
    ...
    ("ch15_imn.py",               "fast",  "IMN phi^2 sums (pi/8; 0.236 tail-corrected); Casimir negative result; spacelike commutator"),
    ...
    ("ch21_friedmann.py",         "slow",  "Friedmann ladder |Delta alpha| ~ 0.009; exponents 2/3, 1/2; update-interval check"),
...
def main():
    ...
    ap.add_argument("--report", action="store_true",
                    help="capture output and write ../data/provenance.json")
    args = ap.parse_args()
    ...
    for script, tier, what in selected:
        path = HERE / script
        if not path.exists():
            print(f"[skip ] {script:30s} (MISSING) — {what}")
            failures.append(script)
            entries.append({"script": script, "tier": tier, "validates": what,
                            "status": "MISSING"})
            continue
        print(f"[run  ] {script:30s} [{tier}] — {what}")
        t0 = time.time()
        r = subprocess.run([sys.executable, str(path)], cwd=HERE,
                           capture_output=args.report, text=True)
        dt = time.time() - t0
        status = "ok" if r.returncode == 0 else "FAIL"
        print(f"[{status:5s}] {script:30s} ({dt:.1f}s)")
        if r.returncode != 0:
            failures.append(script)
            if args.report and r.stderr:
                print(r.stderr[-2000:])
        entries.append({"script": script, "tier": tier, "validates": what,
                        "status": status, "seconds": round(dt, 1),
                        **({"printed": r.stdout.splitlines()} if args.report else {})})

    if args.report:
        DATA.mkdir(exist_ok=True)
        report = {"generated": time.strftime("%Y-%m-%d %H:%M:%S"),
                  "tiers": sorted(tiers), "entries": entries}
        json.dump(report, open(DATA / "provenance.json", "w"), indent=1)
        print(f"[write] provenance table -> {DATA / 'provenance.json'}")
