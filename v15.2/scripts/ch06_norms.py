...
def main():
    ...
    print("=== Fermions: Slater/Gram determinant vs product law under contraction ===")
    n1f = 5
    Lppf = (n1f - 1) / n1f
    modes = [1, 2, 3, 4, 6]
    G = np.array([[quad(lambda x, a=a, b=b: 2*np.sin(a*np.pi*x)*np.sin(b*np.pi*x),
                        0, Lppf, limit=300)[0] for b in modes] for a in modes])
    detG = np.linalg.det(G)
    prodN = np.prod(np.diag(G))
    print(f"  n1={n1f}, spectator modes {modes}: det Gram = {detG:.4f}  <=  prod N_con = {prodN:.4f} (Hadamard)")
