...
Prints: 1+1D closed form pi/8L vs partial sums; 3+1D value (partial + tail-corrected);
the Casimir negative result (zero-point sums NOT regulated); the spacelike
commutator table (standard Huygens zero vs mode-volume microcausality violation).
"""
import numpy as np
import matplotlib
matplotlib.use("Agg")
...
def main():
    ...
    ax.set_xlabel("modes kept $N$"), ax.set_ylabel(r"$\langle\phi^2(L/2)\rangle$")
    ax.set_title("1+1D")
    ax.legend(fontsize=8)
    ...
    # --- 3+1D converged value: larger grid + tail correction (tail ~ 1/|n|_max)
    Mbig = 201
    gb = np.arange(1, Mbig + 1, 2)
    Xb, Yb, Zb = np.meshgrid(gb, gb, gb, indexing="ij")
    r2b = (Xb**2 + Yb**2 + Zb**2).astype(float)
    S = np.sum(1.0 / r2b**2)
    tail = (np.pi / 16) / Mbig      # odd-lattice octant density 1/8: sum_{r>R} 1/r^4 ~ (pi/16)/R
    print(f"3+1D <phi^2>(center): partial {4*S/np.pi:.4f} at |n|max={Mbig}, "
          f"tail-corrected {4*(S+tail)/np.pi:.3f} / L0^2 (tail ~ 1/|n|max)")
    ...
    # --- microcausality: unequal-time spacelike commutator, standard vs mode-volume (1+1D)
    print("spacelike commutator [phi(x,t), phi(y,0)] at x=0.70, y=0.20 (separation 0.5 > t):")
    nc = np.arange(1, 40001).astype(float)
    xx, yy = 0.70, 0.20
    uu = 2 * np.sin(nc * np.pi * xx) * np.sin(nc * np.pi * yy)
    dampc = np.exp(-(nc / 20000.0)**2)   # gentle regulator for the conditionally convergent standard sum
    for t in (0.10, 0.30, 0.49):
        std_c = np.sum(uu * np.sin(nc * np.pi * t) / (nc * np.pi) * dampc)
        imn_c = np.sum(uu * np.sin(nc * np.pi * t) / (nc * np.pi * nc))
        print(f"  t={t:4.2f}: standard {std_c:+.1e} (Huygens zero)   mode-volume {imn_c:+.4f} (nonzero: microcausality violated)")
