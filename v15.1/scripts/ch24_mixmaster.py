#!/usr/bin/env python3
"""
ch24_mixmaster.py — Bianchi IX (Mixmaster): Kasner epochs and the BKL map (Ch. 24).

Vacuum Bianchi IX in BKL time tau (dt = abc dtau), A_i = ln a_i:

   2 A1'' = (a2^2 - a3^2)^2 - a1^4   (+ cyclic),
   C = A1'A2' + A2'A3' + A3'A1'
       - (1/4)[a1^4 + a2^4 + a3^4 - 2(a1^2 a2^2 + a2^2 a3^2 + a3^2 a1^2)] = 0.

Between wall activations the dynamics is the Ch. 20 Kasner flight; each bounce
maps u -> u-1 (u >= 2) or 1/(u-1) (1 < u < 2): the BKL map.  We integrate
toward the singularity, extract u per epoch, and test the map.  The constraint
is the run's quality certificate: any run drifting beyond 1e-10 is DISCARDED
(hard gate below, App. C.5).

Adapted from scripts/ports/mixmaster.py.
Writes ../data/ch24_mixmaster_traj.npz; Figure 24.3.
"""
import pathlib
import sys
...
def constraint(y):
    A, P = y[:3], y[3:]
    a2 = np.exp(2 * A)
    W = 0.25 * (a2[0] ** 2 + a2[1] ** 2 + a2[2] ** 2
                - 2 * (a2[0] * a2[1] + a2[1] * a2[2] + a2[2] * a2[0]))
...
print(f"# constraint drift over run: {Cdrift:.2e}   (gate: {DRIFT_GATE:.0e})")
if Cdrift > DRIFT_GATE:
    sys.exit(f"RUN DISCARDED: constraint drift {Cdrift:.2e} exceeds {DRIFT_GATE:.0e}")
...
for i in range(1, len(epochs)):
    upred = bkl(upred)
    diff = epochs[i][2] - upred
    maxdiff = max(maxdiff, abs(diff))
    print(f"  epoch {i+1}: u_measured = {epochs[i][2]:.4f}   BKL prediction = {upred:.4f}"
          f"   diff = {diff:+.4f}")
print(f"# max |u_measured - u_BKL| across epochs: {maxdiff:.1e}")

np.savez_compressed(DATA / "ch24_mixmaster_traj.npz", t=sol.t, A=A, P=P, ptau=ptau,
                    u=u_tau, epochs=np.array([(a, b, c) for a, b, c in epochs]))
...
ax3.contour(P1, P2, P1**2 + P2**2 + (1 - P1 - P2) ** 2, levels=[1.0], colors="k",
            linestyles="--")
...
fig.tight_layout(); fig.savefig(FIGS / "ch24_fig3_mixmaster.png", dpi=150); plt.close(fig)
print("figure -> ch24_fig3_mixmaster.png; data -> ch24_mixmaster_traj.npz")
