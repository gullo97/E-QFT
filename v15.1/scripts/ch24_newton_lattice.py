#!/usr/bin/env python3
"""
ch24_newton_lattice.py — the static weak-field limit on the 3D cell lattice (Ch. 24).

Linearizing the constraint-closed cell dynamics about a uniform background, with
the Ch. 22 gradient stiffness coupling neighbouring cells, the static sector is
the lattice Poisson equation for the log-scale-factor perturbation:

    kappa (lattice Laplacian) delta_alpha(x) = -c_m delta_rho(x),

i.e. delta_alpha = -Phi_Newton (units kappa = c_m = 1; the structure is the
result).  Solved on a 128^3 periodic lattice by FFT:
  (1) point mass  -> delta_alpha(r) ~ 1/r (the Newtonian potential profile);
  (2) two masses  -> binding energy ~ -1/d, force ~ 1/d^2 (inverse square).

Adapted from scripts/ports/newton_lattice.py.  Figure 24.1.
"""
import pathlib
...
def poisson(rho):
    rho = rho - rho.mean()                  # periodic solvability (zero-mode removed)
...
r = rr.ravel()
av = alpha.ravel()
rb = np.linspace(3, 16, 14)                  # fit window excludes small-r discreteness
...
ds = np.arange(4, 19)
Eint = np.array([-alpha[d % Ng, 0, 0] for d in ds])   # E_int = int Phi_1 rho_2 < 0 (binding)
...
print(f"# two masses: slope of |E_int(d)| = {sE:.3f} (Newton: -1);  "
      f"slope of |F(d)| = {sF:.3f} (Newton: -2)")
...
fig.tight_layout(); fig.savefig(FIGS / "ch24_fig1_newton.png", dpi=150); plt.close(fig)
print("figure -> ch24_fig1_newton.png")
