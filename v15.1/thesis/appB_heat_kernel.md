...
**Why η is cleaner than the sums it is built from.** Pair the spectrum at large $|E|$: by Weyl asymptotics the positive and negative branches have, level by level, matching densities up to $\mathcal O(1/|E|)$ corrections, so in $\eta(s)$ the dangerous high-energy tails cancel *pairwise inside the sum* before any limit is taken: $\eta(s)$ has **no bulk divergence** — its small-$s$ expansion is constant + vanishing. By §B.1's criterion the limit is then regulator-independent for every even damping. (This pairing is proved here for the 1+1D bag class; in higher dimensions $\eta(s)$ can develop genuine power divergences, and the statement must be re-earned before the 3+1D program of Ch. 27, item 3, can lean on it.) (Verified three ways on the toy spectra of Ch. 9: heat, Gaussian, normalized-smooth-cutoff regulators extrapolating to the same value — `ch09_eta_toy.py`.) $\eta$ is an *infrared* quantity wearing ultraviolet clothes: it measures the spectrum's imbalance — decided by level crossings near $E = 0$ *and* by the branch-density tilt the boundary data impose.

**Behaviour under deformation: the staircase decomposition.** Deform the operator along a parameter $\lambda$. Two distinct things can move $\eta$, and keeping them separate is the content of Corollary 9.2:

$$\Delta\eta \;=\; \delta\eta_{\text{smooth}} \;+\; 2\times(\text{spectral flow}), \tag{B.3}$$

*integer jumps* — when a single level crosses $E = 0$ its $\operatorname{sgn}$ flips and $\eta$ jumps by $\pm2$, the direction set by the crossing direction — and a *smooth part*, fed by deformations that tilt the positive and negative branches' level densities against each other. The smooth part is the finite-volume shadow of the local (boundary-data) term in the Atiyah–Patodi–Singer variation formula, and it is not exotic: the massless two-wall chiral bag has $E_n^+ = [(n+\tfrac12)\pi - \Delta/2]/L$, $E_n^- = -[(n+\tfrac12)\pi + \Delta/2]/L$, whence $\eta(s) \to \Delta/\pi$ — continuous in the wall mismatch, with the spectral gap open throughout: no level crosses zero, and $\eta$ moves anyway (**[Computed]** two regulators, eight digits, `ch09_eta_toy.py`).

What the pairing argument honestly proves, and what it does not. For fixed $s > 0$ the damped sum converges uniformly on any compact $\lambda$-interval whose gap $\min_k|E_k(\lambda)|$ is bounded below, so $\eta(s, \lambda)$ is continuous in both arguments; but the uniformity is lost as $s \to 0$, so continuity does *not* upgrade to constancy of the limit — the shifted-tower example shows the failure is real, not hypothetical. Constancy — the exact plateau — is *not* a relabeling statement (at $m \ne 0$ a box-size change is not a rescaling, and levels genuinely migrate — the Ch. 12 sub-gap level crosses the whole gap); it is a *locality* statement: the smooth part is the APS local term, an integral of a CP-odd density of the background, which in the bulk requires a chiral-angle gradient (absent for a constant real mass) and at the boundary depends only on the wall data. Deformations that hold the wall data fixed — box size only — therefore have $\delta\eta_{\text{smooth}} = 0$, the fractional part staying pinned at the Goldstone–Wilczek value $-\Delta/2\pi$ (Ch. 12). For that class the plateaus are exact by this argument and measured flat to a few parts in $10^5$ across the working range of the Ch. 12 pump (`ch09_eta_toy.py`); the jumps then carry all the change. That is the quantization mechanism of Ch. 12, with the smooth part parked in the fractional $-\Delta/2\pi$ law.
...
*(Derivation of the second coefficient: Mellin-transform (B.4) at $d = 3$. The trace $\operatorname{Tr}e^{-s\Delta} = \int_0^\infty e^{-s\omega^2}\,dN(\omega)$; inserting $N = A\omega^3 + B\omega$ and using $\int_0^\infty e^{-s\omega^2}\omega^2 d\omega = \tfrac{\sqrt\pi}{4}s^{-3/2}$, $\int_0^\infty e^{-s\omega^2}d\omega = \tfrac{\sqrt\pi}{2}s^{-1/2}$ matches $(4\pi s)^{-3/2}\int\sqrt g\,[1 + \tfrac s6 R]$ term by term: $A = V/6\pi^2$ and $3A\cdot\tfrac{\sqrt\pi}{4} = \tfrac{V}{(4\pi)^{3/2}}$ ✓, then $B\cdot\tfrac{\sqrt\pi}{2}s^{-1/2} = \tfrac{s}{(4\pi s)^{3/2}}\cdot\tfrac16\int\sqrt gR$ gives $B = \tfrac{1}{24\pi^2}\int\sqrt g R$.)* As written, (B.6) is for closed spaces — the periodic lattices of Ch. 25; a boundary would add an $\omega^2$ surface term between the two displayed ones, which none of this appendix's consumers need.
...
**Validation.** §B.1–B.2: `ch09_eta_toy.py` (three-regulator agreement; the staircase decomposition — plateau flatness, jumps, and the smooth part $\eta = \Delta/\pi$ on the massless tower). §B.3–B.4's algebra is closed-form; its consumers validate it wholesale — `ch22_casimir_chain.py` / `ch22_dispersion.py` (the $-\tfrac1{12}$ and the induced stiffness) and `ch25_graviton.py` (the $C(s)$ convergence table).
