...
$$\Delta Q_{\text{cross}} = \!\!\sum_{\substack{k:\,E'_k>0\\ n:\,E_n<0}}\!\! |\alpha_{kn}|^2 \;-\!\! \sum_{\substack{k:\,E'_k<0\\ n:\,E_n>0}}\!\! |\alpha_{kn}|^2, \qquad \Delta Q_{\text{same}} = \!\!\sum_{\substack{k:\,E'_k>0\\ n:\,E_n>0}}\!\! |\alpha_{kn}|^2 \;-\!\! \sum_{\substack{k:\,E'_k<0\\ n:\,E_n<0}}\!\! |\alpha_{kn}|^2. \tag{11.1}$$

— the *cross form* counts particles materializing from the old sea minus antiparticles from the old positive branch; the *same form* follows from it by occupancy bookkeeping: each old mode's weight lands on one new branch or the other, $\sum_{k:\,E'_k>0}|\alpha_{kn}|^2 + \sum_{k:\,E'_k<0}|\alpha_{kn}|^2 = 1$ (the row identity A.7), so trading every cross-branch sum for its same-branch complement and cancelling the equal branch counts turns one form into the other. Note the hypothesis that trade smuggles in: at infinite basis the two forms differ by exactly $\eta(L)$ — the same-form limit is $\tfrac12\operatorname{Tr}(S + S')$, the cross-form limit $\tfrac12\operatorname{Tr}(S' - S)$, in the notation of §11.2 — so they are equal *only because* the old spectrum is mirror-symmetric here (Spectral Mirror Theorem), not by completeness alone. At $n_{\max} = 18$ at the representative point, they disagree by $19\%$ — and that discrepancy, far from being a blemish to average over, is **the smoking gun**: a unitary transformation truncated at finite rank is not unitary, and two expressions equal through (A.7) must split by exactly the defect (A.9). A computation whose two honest forms disagree by 19% is telling you its target has not converged — or is not what you think it is. Both, here.
...
>
> — one half the *new* Hamiltonian's sign function, traced over the *old* region only. The trace is a *regulated* one, like every spectral trace in Part II: the weights do not decay ($w_k \to L/L'$ at large $|k|$; §11.3), so the sum is defined by the heat-kernel limit, converging through the $\pm$ cancellations the regulator organizes.
...
| whole-box $\eta(L'),\ \eta(L)$ | $0$ | $0$ | $0$ |

One honesty note on the last row: in this pipeline the spectra are the analytic roots of $\tan(pL) = -p/(m\cos\delta)$ with $E = \pm\sqrt{p^2 + m^2}$ assembled in pairs, so the whole-box $\eta$ vanishes *by construction* — a consistency wire, not a test; the honest solver-level verification of the mirror is `ch10_mirror_spectrum.py` (Ch. 10.4). The historically quoted value at this point ($+0.069$) is recognizably the same-branch entry of the first column: a truncation snapshot of a polarization integral, taken on its way to $+0.0672$. The $18\%$ inter-form gap at $n_{\max} = 18$ is the truncation defect (A.9) of a rank-18 basis chasing a slowly converging *boundary* integral — the weights themselves do not decay ($w_k \to L/L' = 5/6$ here); only their signed, mirror-cancelled sum settles, at the sinc-tail rate of Ch. 3's overlaps — and Ch. 5's boxed principle applies with full force: an observable that needs the cutoff to mean anything is not the observable you wanted.
...
1. **Net charge = spectral flow. Always.** Compute complete spectra and take $\tfrac12\big[\eta_{\text{after}} - \eta_{\text{before}}\big]$ (Cor. 9.2's convention), or count zero crossings along the deformation. Never report a truncated branch-sum residue as charge — *even when it converges*: the limit exists and is the polarization (11.2), not the charge.
...
**Validation.** `ch11_polarization.py`: `--repro` prints $A_{\text{same}} = +0.0764$ and $A_{\text{cross}} = +0.0619$ at the historical parameter point; the default run prints the convergence table, the polarization value $+0.0672$, and Fig. 11.2; `--oddness` produces Fig. 11.3 ($|\Delta Q(+\delta) + \Delta Q(-\delta)| < 10^{-14}$ across the phase range, both forms); `--defect` prints the App. A truncation-defect scaling quoted in A.4. `ch11_wall_density.py`: regulated vacuum charge density profiles before/after expansion (Fig. 11.1), plus the new vacuum's charge inside the old region, Richardson-extrapolated to $-0.0672$ — the Polarization Identity value with the sign unwound. Both scripts build their spectra from the analytic $\tan(pL) = -p/(m\cos\delta)$ roots, so every whole-box $\eta$ they print vanishes by construction; the honest solver-level mirror test is `ch10_mirror_spectrum.py`.
