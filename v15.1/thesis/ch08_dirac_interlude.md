...
maps solutions to solutions with $E \to -E$ (check: conjugate (8.1), multiply by $\sigma_1$, use $\sigma_1\sigma_1^* = \mathbb 1$, $\sigma_1\sigma_3^*\sigma_1 = -\sigma_3$): the negative branch is the charge mirror of the positive one. **[Standard]** This little map has two big appearances ahead: it forces the symmetric ordering of §8.7, and it is compatible with the MIT walls (it maps each wall ray of §8.3 to itself), which pairs the real-mass bag spectrum into mirror twins. A caution for later: once a CP phase enters the mass, $C$ relates the $+\delta$ problem to the $-\delta$ problem rather than acting within one spectrum — the mirror symmetry of Ch. 10 will have to come from the boundary algebra itself, not from $C$.

## 8.3 Confinement without infinite potentials: the MIT bag

How does one put a Dirac particle in a box? The Schrödinger reflex — demand $\psi = 0$ at the wall — fails: (8.1) is first order, so fixing *both* components at a point generically forces $\psi \equiv 0$ (one condition per wall is all a first-order system can bear). The electrostatic reflex — a steep vector-potential wall — fails more interestingly: a step higher than the gap produces transmission rather than confinement, by pair creation across it (the Klein paradox), the relativistic vacuum's way of announcing that it cannot be pushed around by potentials that couple to charge. What *does* confine is a Lorentz-*scalar* wall — a position-dependent mass $m(x) \to \infty$ — and in that limit the wall's entire effect collapses to a *local, linear boundary condition* on $\psi$, whose form is fixed by current conservation: no probability flux through the wall,

$$n_\mu\,\bar\psi\gamma^\mu\psi\big|_{\text{wall}} = 0, \tag{8.4}$$

implemented by a *local, linear* condition on $\psi$. The minimal solution — and precisely the $m(x) \to \infty$ scalar-wall limit — is the **MIT bag boundary condition** **[Standard]**:
...
(App. A states it in the operator form used throughout and tracks what happens when sums are truncated — the technical root of Ch. 11). The Hamiltonian $\hat H = \sum_k E_k\,\hat c^\dagger_k \hat c_k$ is unbounded below as it stands; the stable construction is Dirac's: define the vacuum by *filling the sea*,
...
Charge conjugation $C$ exchanges $\psi$ and its conjugate; the physical charge must be exactly $C$-odd, $\;\hat Q \xrightarrow{C} -\hat Q$, *as an operator identity, independent of any vacuum*. Among instantaneous local bilinears the combination with that property is unique (up to normalization): the antisymmetrized, symmetrically ordered

$$\hat Q \;=\; \frac12 \int dx\,\big[\psi^\dagger\psi \;-\; \psi\,\psi^\dagger\big] \;=\; \frac12\int dx\, \big[\psi^\dagger,\psi\big]. \tag{8.14}$$
...
Now expand (8.14) in a complete bag eigenbasis. Write the field as

$$\psi(x) = \sum_{E_k > 0} \hat b_k\, \psi_k(x) + \sum_{E_k < 0} \hat d_k^\dagger\, \psi_k(x),$$

annihilators $\hat b_k$ for particles, *creators* $\hat d_k^\dagger$ for the negative branch (a hole created is an antiparticle). Insert into (8.14) and use orthonormality of the $\psi_k$:
...
$$\boxed{\;\hat Q \;=\; \sum_{E_k>0} \hat b_k^\dagger \hat b_k \;-\; \sum_{E_k<0} \hat d_k^\dagger \hat d_k \;+\; Q_{\text{vac}}, \qquad Q_{\text{vac}} \;=\; -\,\frac12\Big(\sum_{E_k>0}\!\!1 \;-\; \sum_{E_k<0}\!\!1\Big) \;=\; -\,\frac12\,\eta\;} \tag{8.15}$$
...
Equation (8.15) is the hinge of Part II, so let us state what it says with full force. The charge of a state is *particles minus antiparticles* — the familiar part — **plus a c-number that belongs to the vacuum itself**, fixed entirely by the spectrum of the Dirac operator. If the spectrum is symmetric under $E \to -E$, the vacuum is neutral and the naive counting is safe. If the spectrum is lopsided — one level pushed across zero, say — the vacuum *carries charge*, with no particle anywhere. Both the divergence of each sum in $Q_{\text{vac}}$ and its regularization are dealt with honestly in Ch. 9; the structure is what matters here.

## 8.8 Charge bookkeeping across a quench

One more preparation, cheap now and decisive later. Let the box change suddenly, $L \to L'$. Two complete eigenbases now exist — old and new — and expression (8.15) can be evaluated in either. The operator (8.14), however, is built from the field alone: *it does not know which basis we expand it in*. Moreover the quench Hamiltonian is bilinear in the field and commutes with $\hat Q$ — a sudden change of boundary parameters conserves total charge. So one conserved expectation value can be written two ways:

$$\langle \hat Q\rangle \;=\; \underbrace{\big(N_b - N_d\big)_{\text{old}} + Q_{\text{vac}}(L)}_{\text{old basis}} \;=\; \underbrace{\big(N_b - N_d\big)_{\text{new}} + Q_{\text{vac}}(L')}_{\text{new basis}} . \tag{8.16}$$

Everything Part II needs is already visible in (8.16): if the *measured* particle–antiparticle imbalance is counted against the new vacuum while the state began as the old vacuum, the imbalance is fixed by the *difference of vacuum charges* — a purely spectral quantity. Chapter 9 promotes this two-line observation to the master theorem of the thesis and draws its sweeping corollaries; Chapters 10–12 then ask the only remaining question, which turns out to have a sharp and surprising answer: *what does it take to make $Q_{\text{vac}}$ actually change?*
