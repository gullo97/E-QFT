...
Part II closes with a diagnostic chapter of a different flavor: not a wrong *answer* this time, but a tempting wrong *implementation* of the framework's own central idea. The question it answers is one any thoughtful reader has by now: the iso-energy structure ties mode numbers to box sizes — could that tie live *inside* the field operator of an ordinary fixed-geometry QFT, say as a mode-dependent normalization, and incidentally cure the ultraviolet? The proposal is tempting, its successes are real and striking (every propagator-type divergence dies, in every dimension) — though §15.1 shows its iso-energy pedigree is already backwards on arrival — and it fails, definitively, by theorem, in a way that illuminates exactly *why* the arena of Chapter 3 has the shape it has. This is the cheapest available proof that geometry must be dynamical, and the thesis spends a chapter on it for the same reason Ch. 11 dissected the polarization: the strongest defense of the right structure is the autopsy of its nearest plausible rival.

## 15.1 A tempting shortcut

In the iso-energy framework, mode $n$ is tied to geometry — but one must be careful about *which way the tie points*. The ladder through $(n, L_0)$ has rungs $(k, kL_0/n)$; its ground rung is the *smaller* box $L_0/n$, so a normalization that honored the iso-energy relation would carry $V_n = (L_0/n)^d$ and *enhance* high modes, making the ultraviolet worse. The shortcut this chapter dissects points the ladder the other way: it assigns mode $n$ the volume of the $n$-th rung of the ladder based at $(1, L_0)$ — the box $nL_0$, which carries the *ground state's* wavelength, not mode $n$'s. That assignment is not derived from the iso-energy relation; it is an ad-hoc ultraviolet cure wearing iso-energy clothing, chosen because the damping direction is the one that regulates. We say this bluntly at the outset because the proposal's pedigree matters to the autopsy. Standard quantization, for its part, normalizes every mode by the *same* volume factor $1/\sqrt{L_0^d}$. The shortcut proposes: let each mode carry its assigned partner volume. Define the **mode-volume normalization** of a massless scalar in $[0, L_0]^d$:
...
$$\langle 0|\hat\phi^2(L_0/2)|0\rangle_{\text{mvn}} = \sum_{n\;\text{odd}}\frac{1}{\pi n^2} = \frac{1}{\pi}\cdot\frac{\pi^2}{8} = \boxed{\frac{\pi}{8}} \tag{15.2}$$

— dimensionless, as $\langle\phi^2\rangle$ in 1+1D must be (the general-$d$ scaling is $L_0^{1-d}$: compare the 3+1D value $0.236/L_0^2$ below) — a closed form. **[Computed]** partial sums to $N = 5000$ give $0.392667$ against $\pi/8 = 0.392699$ — four parts in $10^5$ (`ch15_imn.py`). In 3+1D the standard sum diverges quadratically ($\sum 1/|\vec n|$ over the odd lattice); the prescription turns it into $\sum 1/|\vec n|^4$, convergent — slowly, with a $1/|\vec n|_{\max}$ tail — to $\approx 0.236/L_0^2$ (partial sum $0.2349$ at $|\vec n|_{\max} = 201$, tail-corrected $0.236$; `ch15_imn.py`). And the pattern is dimension-blind: the standard radial integrand $k^{d-2}$ becomes $k^{d-2}\cdot k^{-d} = k^{-2}$ — *one* uniform UV behaviour for every $d$.

**Loop power counting collapses.** At fixed physical momentum the damping reads $(|\vec k|L_0/\pi)^{-d}$ per propagator, so every internal line of a Feynman graph gains $|\vec k|^{-d}$ and the superficial degree of divergence drops to $\omega_{\text{mvn}} = \omega_{\text{std}} - d\cdot I$. In $d = 3$, the electron self-energy (superficial degree $\omega_{\text{std}} = 1$; the physical divergence is logarithmic after the standard cancellation) lands at $\omega = 1 - 6 = -5$: *finite*. The oldest embarrassment of quantum field theory — the divergent electromagnetic mass — would simply be a number.
...
*Figure 15.2 — Convergence where there was divergence: 1+1D partial sums saturating at $\pi/8$; 3+1D sums saturating (slowly — the tail decays as $1/|\vec n|_{\max}$) while the standard sums climb away.*
...
>
> Moreover the field violates microcausality, and the diagnosis must be stated carefully. At equal times $[\hat\phi(\vec x), \hat\phi(\vec y)]$ still vanishes identically (the $\pm\omega$ terms cancel mode by mode, damping or not), but $[\hat\phi(\vec x), \hat\pi(\vec y)]$ is the smoothed delta of §15.3 — nonlocal support already at the canonical level. The decisive object is the *unequal-time spacelike* commutator $[\hat\phi(x,t), \hat\phi(y,0)]$ with $|x - y| > t$, which the standard field's Huygens cancellation forces to zero and the damped mode sum does not: **[Computed]** at separation $0.5$ (1+1D, $L_0 = 1$), the standard sum gives $10^{-8}$–$10^{-6}$ (regulator-limited zero) while the mode-volume field gives $0.0338$, $0.1132$, $0.2615$ at $t = 0.1, 0.3, 0.49$ (`ch15_imn.py`). The field is a **generalized free field violating microcausality** — unitary at the free level, but with no local interacting extension.
...
**Validation.** `ch15_imn.py`: the $\pi/8$ closed form (partial sums to $N = 5000$: $0.392667$ vs $\pi/8 = 0.392699$), the 3+1D value (partial $0.2349$ at $|\vec n|_{\max} = 201$, tail-corrected $0.236$), the undamped zero-point sums, and the spacelike-commutator table (standard $10^{-8}$–$10^{-6}$ vs mode-volume $0.0338$–$0.2615$). Figure 15.3 by `make_schematics.py`.
