...
a single band of width $4J$. (With the hopping written as $+J$ in (4.1) the eigenvalue reads $+2J\cos q$; the relabeling $q \to \pi - q$ maps one convention onto the other over the same family of states — we adopt the sign that puts the band minimum at $q = 0$, matching the figure.) Group velocity $v_g(q) = 2J\sin q$, maximal at band center $q = \pi/2$ — the fastest a geometry packet can travel is two rungs per unit of $1/J$ time, an emergent "speed limit" on size change. Two glosses, each consumed later:

*What $q$ is.* The quasimomentum conjugate to ladder *position* — i.e., to the size of the universe. It is not a momentum of anything material; it is the phase gradient of a superposition across geometries. Chapter 20's fixed points $\theta = \pm\pi/2$ are the band-center states of maximal size-change speed (its $\theta$ is this quasimomentum in the raw $+J$ convention, $\omega = +2t\cos\theta$; at band center the two sign conventions coincide, and Ch. 20 — working with packets far from the floor — uses both signs of $\theta$ as the expanding and contracting directions).
...
>
> with $J_d$ the Bessel function of order $d$. *Derivation in three lines:* for this $\hat H$ the Bloch energies are $E(q) = 2J\cos q$ — the raw $+J$ convention, *not* §4.2's relabeled $q$; the two differ by $q \to \pi - q$, and the $(-i)^d$ in (4.4) belongs to this one — so the propagator is diagonal, $e^{-i(2J\cos q)t}$; transforming back gives $\int_{-\pi}^{\pi}\frac{dq}{2\pi}e^{iqd}e^{-2iJt\cos q}$, which is exactly the integral representation of $(-i)^d J_d(2Jt)$. (On the half-infinite ladder, add the image term $-(-i)^{d'}J_{d'}(2Jt)$, $d' = n + n_0$, enforcing the floor node — negligible for packets far from $n = 1$.) The Bessel function's two regimes are the physics: for $d < 2Jt$, oscillatory — the packet has arrived and interferes; for $d > 2Jt$, super-exponentially small — causally not yet reachable. The edge $d = 2Jt$ is the ballistic front moving at $v_g^{\max}$.
...
**Validation.** `ch04_ladder_dynamics.py`: exact evolution vs the Bessel propagator (machine agreement off the floor); the carpet plot (Fig. 4.2); band dispersion check; participation ratios ($\mathcal R(1) = 2.52 \to 1$); Figs. 4.1, 4.3.
