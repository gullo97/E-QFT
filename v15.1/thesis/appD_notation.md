...
| $J_{CP}$, $\mathcal S_{\text{GIM}}$ | Jarlskog invariant; GIM mass-difference suppression (the $\varepsilon_{CP}$ ladder of Ch. 14.4 is built from these plus loop measures) | Ch. 13/14 |
...
| $g_{ab} = e_a \cdot e_b = \sum_i e^i_a e^i_b$ | cell metric as the Gram matrix of edge vectors (triad $e_a$; $a, b$ edge labels, $i$ ambient — Ch. 18's convention; Ch. 16.9's frame-index version is $g_{ij} = e^a_i e^a_j$, the index roles swapped as §18.3 flags) | Ch. 18 |
...
- $q$: below-gap momentum $p = iq$ (Part II) vs ladder quasimomentum (Ch. 4) vs perturbation wavenumber of $\Pi(\omega, q)$ and the block-spectral sweeps (Ch. 25). Never two in one equation; first use in each chapter names its meaning. Ch. 20 writes the ladder quasimomentum $\theta$ (its semiclassical phase-space variable, raw $+t$ convention — flagged at (20.1)); this collides typographically with Part II's wall angles $\theta_0, \theta_L$, which never appear in the same chapter.
- $c$: lattice constant in $\Lambda_{\text{UV}} = c/\bar L$ (Ch. 22/23) vs the matter coupling of the lattice Poisson equation, always written $c_m$ (Ch. 24). The speed of light is $1$ throughout.
...
**Kasner circle** — the locus $\sum p_a = \sum p_a^2 = 1$ of vacuum Bianchi-I exponents; every point but one has a contracting axis. The generator-coupled ladder lands on it to three digits ($\Sigma_K = 0.9997$; the FRW control point is the four-digit one, $0.3333$). (Ch. 16.6, 20.)
...
Generated programmatically: `run_all.py --report` writes the provenance table (script → what it validates → status → printed numbers) into `data/provenance.json`; the file list with status lives in `coverage_matrix.md`.
