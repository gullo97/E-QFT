...
A single particle on the geometry ladder moves symmetrically: nothing in Chapters 3–4 distinguishes expansion from contraction. This chapter adds the one ingredient the real universe insists on — *other particles* — and proves that the symmetry shatters. When an active particle takes an iso-energy step, every other particle in the box (every **spectator**) must come along for the ride, and the price of the ride is direction-dependent: free for expansion, costly for contraction, and *multiplicatively* costly across spectators. The ratio of contraction to expansion probability falls exponentially with the number of particles. For any macroscopic occupancy — and, near the quantum floor, already for a few dozen particles — contraction effectively never happens (the worked thresholds scale as $N_{\text{crit}} \sim 14\,n_1$; §6.6).
...
The chapter has a second purpose, equally important for the thesis's defensibility. Two ingredients of this argument were, historically, *postulates*: the rule that an unrenormalized projection norm is a transition probability, and the hypothesis that spectator contributions factorize. Section 6.5 proves both — they are short-time first-order perturbation theory in the sector arena of Ch. 3. The arrow of expansion thereby moves from "well-motivated model" to "theorem within the framework", and the assumptions that remain are exactly those flagged in Ch. 3 plus the regime: the sector arena itself, the form of the inter-sector coupling, and the short-time window in which probabilities are read off (§6.5, scope note).
...
So expansion asks nothing of the spectators: unit probability, energy intact. The "iso-energy" label of the transition is earned, for expansion, by the whole assembly and not just the active particle. **[Computed]** `ch06_norms.py` verifies (6.1) to the $10^{-6}$ printed precision and (6.2) to the $6\times10^{-5}$ accuracy that 4000-mode sums allow — the KE sum rule's tail decays only as $1/k_{\max}$ — across a grid of $(n_2, s)$.
...
>
> whenever $\varphi_{n_2}(L'') \neq 0$ — every spectator mode except the *resonant* ones, $n_2$ a multiple of $n_1$ (one mode in every $n_1$).
...
>
> $$P_{\text{transfer}}(t) \;=\; (gt)^2\,\big\|\,\iota_{(N+1)}\,\Psi\,\big\|^2 + o(t^2), \qquad \iota_{(N+1)} = \iota^{\otimes(N+1)}, \tag{6.6}$$
>
...
The remainder in (6.6) is written $o(t^2)$ deliberately, not $\mathcal O(t^4)$: the would-be $t^4$ coefficient involves $\sum_f |\langle f|\iota\Psi\rangle|^2 (E_f - E_i)^2$, which *diverges* for contraction — that is Theorem 6.4 speaking — so the approach to the $t^2$ law is a slower power, measured below.
...
*Scope, stated plainly.* Despite the traditional name, this is a **short-time embedding theorem**: it is proved in the quadratic ($t^2$) window, for the postulated unrenormalized embedding coupling (6.5), and what it converts into rates are short-time transfer probabilities. In the linear-in-$t$ golden-rule regime, energy conservation reweights final states and neither the total-norm reading of (6.6) nor the factorization (6.7) follows in this clean form; the direction of the asymmetry is expected to survive there (contraction's final states are pushed *further* off-shell by their divergent kinetic energies), but we do not prove it. Everything downstream — the arrow, Ch. 7's breaking, Ch. 14's departure factor — consumes the short-time statement.

**[Computed]** (`ch06_golden_rule.py`, two-sector model, 220 modes per sector): Parseval sums reproduce $\mathcal N_{\text{con}}$ — $0.76749$ vs analytic $0.76882$ for $(n_1, n_2) = (5,3)$; $0.85112$ vs $0.85249$ for $(8,5)$; $0.90373$ vs $0.90530$ for $(12,7)$ — with residuals scaling as $1/n_{\max}$ exactly as the sinc tails dictate; expansion gives $1.00000$. Exact unitary evolution of the coupled two-sector Hamiltonian, no perturbation theory, gives $P(t)/(gt)^2 \to$ the embedding norm from below as $t \to 0$ — measured for the $(n_1, n_2) = (5, 3)$ contraction: $0.682 \to 0.761$ across a hundredfold decrease of $t$, against the basis-truncated norm $0.767$ — confirming both the theorem and its claimed regime of validity. The approach is slow, and honestly so: the deficits $0.767 - 0.682 = 0.085$ and $0.767 - 0.761 = 0.006$ across that hundredfold drop scale as $\sim t^{0.6}$ — the $\sqrt t$-type law expected because the correction is controlled by $\sum_f |\langle f|\iota\Psi\rangle|^2\Delta E_f^2$, divergent by Theorem 6.4. This is exactly why (6.6) claims $o(t^2)$ and not $\mathcal O(t^4)$: the energetic violence of sudden contraction is visible even in how its rate converges.
...
> **Expansion-Preference Theorem [Theorem].** Define $P_{\text{exp}}$ (resp. $P_{\text{con}}$) as the short-time probability, under the coupling (6.5), of the event *the active particle in mode $n_1$ completes its iso-energy step* — arrives in its promoted (resp. demoted) target mode — *with the $N$ spectators unrestricted*. Then
>
...
One bookkeeping remark, so that no convention hides in the prefactor. If one instead compares *total* sector-transfer probabilities — active particle unrestricted too — the expansion norm is exactly $1$ (isometry) and the prefactor becomes $1 - 1/n_1$ instead of $1 - 1/n_1^2$; the contraction side is unchanged, because the restricted active state carries *all* of its surviving weight in the demoted mode (its projection onto the target, $(n_1{-}1)/n_1$, equals its full restriction norm). Both conventions carry the same product of spectator deficits and the same exponential in $N$; we quote the iso-energy-restricted one because it prices exactly the step the selection rule defines.
...
**Established at theorem strength, given [Postulate: sector arena] + coupling (6.5), in the short-time regime:** the probability rule, the factorization, the per-spectator deficits, the exponential ratio (6.8)–(6.9), and energy bookkeeping (6.2), (6.4). The historic gap between "norm inequality" and "rate suppression" is closed there — and only there; see the scope note of §6.5.
...
**On statistics.** For fermions (Part II's case) the expansion side is unchanged: $\iota$ is an isometry, so a Slater determinant of distinct modes keeps unit norm. Under contraction, however, the factorization (6.7) fails as an *equality*: the norm of the restricted determinant is the Gram determinant $\det G$, $G_{\alpha\beta} = \langle R\psi_\alpha, R\psi_\beta\rangle$, and the restricted orbitals are no longer orthogonal. What survives is the *inequality*, by Hadamard: $\det G \le \prod_\alpha \mathcal N_{\text{con}}(n_1, n_{2,\alpha})$ — fermionic statistics suppresses contraction at least as strongly as the product law, and generically more. **[Computed]** for $n_1 = 5$ and spectator modes $\{1,2,3,4,6\}$: $\det G = 0.2320$ against the product $0.3895$ (`ch06_norms.py`). Pauli blocking of final states disfavors contraction further still. We use only the direction of these statements, never their magnitude.
...
**Validation.** `ch06_norms.py`: closed forms (6.1)–(6.3), the deficit map (Fig. 6.2), sum rule (6.2), divergence rate of (6.4), the fermionic Gram-determinant check ($0.2320 \le 0.3895$), the asymmetry plot (Fig. 6.4) with $\sigma$ and $N_{\text{crit}}$. `ch06_golden_rule.py`: two-sector Parseval vs analytic table, exact-evolution convergence (Fig. 6.3). Figure 6.1 (spectator projection coefficients) is produced by `regen_part1_figs.py`.
