...
The theorem says the iso-energy lattice is *globally consistent*: the final configuration depends only on the endpoints, never on the path history along one axis. (With several axes the bare operators commute as well — Ch. 18 — though *physical amplitudes* need not be path-independent; see §3.7.) It also justifies equation (3.2): the reachable sizes from $(n_0, L_0)$ are exactly $L_k = (n_0{+}k)L_0/n_0$ — upward for $k \ge 1$ via $\hat T_+$, and symmetrically *downward* via $\hat T_-$ for $-n_0 < k < 0$, the same telescoping run in reverse, reaching every rung down to $L_0/n_0$ — which is why the sector lattice $\mathcal L$, both halves of it, could be written down before any dynamics.
...
Two checks worth recording. **[Computed]** Completeness: $\sum_k |c_k(n,s)|^2 = 1$ for every $(n, s)$ tested (deficits $8\times10^{-12}$ to $9\times10^{-11}$ at 4000 modes, set by the $1/k^4$ tails; `ch03_overlaps.py`) — as Lemma 3.1(a) demands. And explicitly *no Bessel functions appear*: the closed forms are square roots and sincs. (Bessel functions do appear in this thesis — in the *time evolution* on the ladder, Ch. 4 — and keeping the two appearances apart avoids a recurring confusion.)
...
>
> $$\Big\langle n{+}1\Big|\,x\frac{d}{dx}\,\Big|n\Big\rangle \;=\; \frac{2n(n+1)}{2n+1} \;=\; \Big(n + \tfrac12\Big) - \frac{1}{2(2n+1)} \;\approx\; n + \tfrac12 . \tag{3.13}$$

**[Computed]** `ch03_overlaps.py` checks (3.13) at $n = 10, 40, 160$: values $10.4762$, $40.4938$, $160.4984$ against $2n(n{+}1)/(2n{+}1)$ — exact to machine precision.
...
**Conservation, stated honestly.** On a fixed sector, $\hat D$ is not conserved — boxes break scale invariance, which is the whole point. The exact statement tying $\hat D$ to the spectrum's homogeneity is the exponentiated action above: $e^{-i\epsilon\hat D}$ carries every box-$L$ eigenfunction to the box-$e^{\epsilon}L$ eigenfunction of the *same* mode number — $\hat D$ generates the flow along the level sets of the energy, which is the only sense of "conservation" available on a discrete lattice of geometries. **[Theorem]** This is the precise content we will need; we do not claim more. One bookkeeping point must be flagged rather than dressed up: a matrix element of $\hat D$ *between sectors* is not defined until an identification map between the Hilbert spaces is chosen. The Lemma above is computed within a single box; its later use as an inter-sector coupling (Ch. 4, Ch. 20) adopts the fixed-coordinates identification — exactly the kind of choice that §3.7 insists must be declared as part of the physics, so we declare it here.
...
**Validation.** `ch03_overlaps.py`: prints the $\mathcal O_{n\to n+m}$ table, verifies (3.8) against direct quadrature, completeness (deficits $8\times10^{-12}$–$9\times10^{-11}$ at 4000 modes), and (3.13) at $n = 10, 40, 160$. Figure 3.1 is regenerated by the same script; Figure 3.3 by `regen_part1_figs.py`; Figure 3.2 (telescoping schematic) is a carried-over drawing not yet regenerated by the suite.
